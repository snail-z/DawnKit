...
//  zhDatabaseCore
//
//  Created by gren-beans on 08/03/2017.
//  Copyright (c) 2017 gren-beans. All rights reserved.
//
...
- (void)commonInitialization
{
    if (!_model) {
        ...
        [_model zh_mapColumnCreateTableWithPrimaryKeyAndUniqueKeys:@"primary key(songID)"];
    }
...
- (void)example2
{
    ...
    NSArray *models = [_model zh_mapColumnSelectByCriteriaWithFormat:nil];
    [self next:models];
}

- (void)example3
{
    NSArray *models = [_model zh_mapColumnSelectByCriteriaWithFormat:@"isRock = '1'"];
    [self next:models];
}

- (void)example4
{
    NSArray *models = [_model zh_mapColumnSelectOrderKey:@"songID"
                                                descSort:YES
                                              limitCount:3
...
- (void)example5
{
    ...
    [_model zh_updateKeys:@"singerName = '林忆莲'" byCriteria:@"songID = '8'"];
    
    NSArray *models = [_model zh_mapColumnSelectByCriteriaWithFormat:@"songID = '8'"];
    [self next:models];
}

- (void)example6
{
    if ([_model zh_deleteByCriteriaWithFormat:nil]) {
        NSLog(@"成功清空表数据");
    }
}
...
- (void)example7
{
    ...
    BOOL res = [_model zh_deleteByCriteriaWithFormat:@"songID > '2' and songID < '6'"];
    if (res) {
        NSLog(@"删除成功");
        [self example2];
    }

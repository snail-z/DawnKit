...
/// Toast全局管理者
public class ANToastManager: NSObject {
    
    public static let shared: ANToastManager = ANToastManager()
    
    /// 设置Toast外观样式
    public var style: ANToastStyle = ANToastStyle()
...
/// Toast样式
public class ANToastStyle: NSObject {
...
public extension PKViewExtensions where Base: UIView {
    ...
    func showToast(message: String?,
                   ...
                   placement: ANToastManager.ImagePlacement = ANToastManager.shared.placement,
                   position: ANToastManager.ToastPosition = ANToastManager.shared.position,
                   style: ANToastStyle = ANToastManager.shared.style) {
    ...
    private func _messageToast(message: String, delay: TimeInterval, position: ANToastManager.ToastPosition, style: ANToastStyle) {
    ...
    private func _imageToast(image: UIImage, delay: TimeInterval, rotateAnimated: Bool, position: ANToastManager.ToastPosition, style: ANToastStyle) {
        let contentView = UIView()
        contentView.isUserInteractionEnabled = false
        contentView.addShadow(radius: style.shadowRadius, opacity: 1, offset: .zero, color: style.shadowColor)
        base.addSubview(contentView)
    ...
    private func animationOptionsValue(for tag: Int) -> ANToastStyle.AnimationOptions{
        switch tag {
    ...
    func tagValue(for options: ANToastStyle.AnimationOptions) -> Int {
        switch options {
    ...
    private func _perfectToast(text: String, image: UIImage, delay: TimeInterval, rotateAnimated: Bool,
                               placement: ANToastManager.ImagePlacement, position: ANToastManager.ToastPosition, style: ANToastStyle) {
...
//        hud.backgroundColor = .gray
        ...
        shadowHud.pk.makeConstraints { make in
            make.edges.equalTo(hud.pk.edges)
        }
    ...
    private func _adjustToast(_ toast: UIView, _ position: ANToastManager.ToastPosition) {
        switch position {
    ...
    func _hideToast(_ toast: UIView, style: ANToastStyle) {
...
//            toast.transform = CGAffineTransform(scaleX: 0.85, y: 0.85)
    ...
    func _sshideToast(_ toast: UIView, options: ANToastStyle.AnimationOptions, duration: TimeInterval) {
        guard base.pk_activeToasts.contains(toast) else { return }
        let closure: ((Bool) -> Void)? = { _ in
            toast.removeFromSuperview()
            self.base.pk_activeToasts.remove(toast)

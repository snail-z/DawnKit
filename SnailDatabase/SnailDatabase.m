...
#import "NSString+SnailDatabase.h"
...
@interface SnailDatabase ()

@property (nonatomic, strong) NSDictionary<NSString*, FMDatabaseQueue*> *queueDict;

@end

@implementation SnailDatabase

+ (instancetype)sharedInstance {
    static SnailDatabase *_instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _instance = [[SnailDatabase alloc] init];
    });
    return _instance;
}

- (FMDatabaseQueue *)dbQueueAtPath:(NSString *)path {
    SnailDatabaseFile *file = [SnailDatabaseFile fileWithPathName:path];
...
- (FMDatabaseQueue *)databaseQueueAtPath:(NSString*)pathName {
    ...
    if (!object) {
        @synchronized ([SnailDatabase class]) {
...
- (void)inDatabase:(id<SnailDatabaseProtocol>)object database:(void (^)(FMDatabase *))block {
    NSString *pathName = [object respondsToSelector:@selector(sl_databaseName)] ? object.sl_databaseName : self.databaseNameDefault;
...
- (void)inDatabase:(id<SnailDatabaseProtocol>)object transaction:(void (^)(FMDatabase *, BOOL *))block {
    NSString *pathName = [object respondsToSelector:@selector(sl_databaseName)] ? object.sl_databaseName : self.databaseNameDefault;
...
- (BOOL)tableExists:(id<SnailDatabaseProtocol>)object {
    ...
    [self inDatabase:object database:^(FMDatabase * _Nonnull db) {
        result = [db tableExists:object.sl_tableName];
    }];
    return result;
}
...
- (BOOL)dropTableWithObject:(id<SnailDatabaseProtocol>)object {
    ...
    [self inDatabase:object database:^(FMDatabase *db) {
        NSString *sql = [NSString dropTable:object.sl_tableName];
        result = [db executeUpdate:sql];
        if (!result) {
            printError(@"drop failed", object.sl_tableName, db.lastErrorMessage, sql);
        }
    }];
    return result;
}
...
- (BOOL)renameTableWithObject:(id<SnailDatabaseProtocol>)object newName:(NSString *)newName {
    __block BOOL result = NO;
    [self inDatabase:object database:^(FMDatabase *db) {
        NSString *sql = [NSString renameTable:object.sl_tableName newName:newName];
        result = [db executeUpdate:sql];
        if (!result) {
            printError(@"rename failed", object.sl_tableName, db.lastErrorMessage, sql);
        }
    }];
    return result;
}
...
- (BOOL)createTableWithObject:(id <SnailDatabaseProtocol>)object {
    NSAssert([object respondsToSelector:@selector(sl_columnDescription)],
             @"非映射方法创建表需要实现sl_columnDescription方法!");
    NSDictionary *takeDict = [self takeColumnTypeWithDescription:object.sl_columnDescription];
    NSArray *takeColumn = takeDict.allKeys, *takeTypes = takeDict.allValues;
    return [self createTableWithObject:object description:object.sl_columnDescription allColumn:takeColumn types:takeTypes];
}

- (BOOL)createTableWithObject:(id <SnailDatabaseProtocol>)object description:(NSString *)description allColumn:(NSArray *)allColumn types:(NSArray *)allTypes {
    __block BOOL result = NO;
    [self inDatabase:object database:^(FMDatabase *db) {
        NSString *sql = [NSString createTable:object.sl_tableName columnDescription:description];
        result = [db executeUpdate:sql];
        if (!result) {
            printError(@"create failed", object.sl_tableName, db.lastErrorMessage, sql);
        }
        
        NSMutableArray *existColumn = [NSMutableArray array];
        FMResultSet *resultSet = [db getTableSchema:object.sl_tableName];
        ...
        if (existColumn.count != allColumn.count && existColumn.count > 0) {
            ...
            for (NSString *column in newColumns) {
                ...
                NSString *alterSql = [NSString alterTable:object.sl_tableName newColumn:column type:columnType];
                if (![db executeUpdate:alterSql]) {
                    result = NO;
                    printError(@"alter failed", object.sl_tableName, db.lastErrorMessage, alterSql);
                }
            }
        }
    }];
...
- (BOOL)saveObject:(id<SnailDatabaseProtocol>)object values:(NSArray *)values {
    NSString *sql = [NSString replaceIntoTable:object.sl_tableName column:nil columnCount:values.count];
...
- (BOOL)saveIgnoreObject:(id<SnailDatabaseProtocol>)object values:(NSArray *)values {
    NSString *sql = [NSString ignoreIntoTable:object.sl_tableName column:nil columnCount:values.count];
    return [self saveObject:object values:values sql:sql];
}

- (BOOL)saveObject:(id<SnailDatabaseProtocol>)object values:(NSArray *)values sql:(NSString *)sql {
    __block BOOL result = NO;
    [self inDatabase:object database:^(FMDatabase *db) {
        NSLog(@"%@", sql);
        result = [db executeUpdate:sql withArgumentsInArray:values];
        if (!result) {
            printError(@"save failed", object.sl_tableName, db.lastErrorMessage, sql);
        }
    }];
    return result;
}
...
- (BOOL)deleteObject:(id<SnailDatabaseProtocol>)object byCriteriaWithFormat:(NSString *)format, ... {
    ...
    __block BOOL result = NO;
    [self inDatabase:object database:^(FMDatabase *db) {
        NSString *sql = [NSString deleteTable:[object sl_tableName] criteria:criteria];
        result = [db executeUpdate:sql];
        if (!result) {
            printError(@"delete failed", object.sl_tableName, db.lastErrorMessage, sql);
        }
    }];
    return result;
}
...
- (BOOL)deleteObject:(id<SnailDatabaseProtocol>)object sqlWithFormat:(NSString *)format, ... {
    ...
    [self inDatabase:object database:^(FMDatabase *db) {
        result = [db executeUpdate:sql];
        if (!result) {
            printError(@"(sqlFormat)delete failed", object.sl_tableName, db.lastErrorMessage, sql);
        }
    }];
    return result;
}
...
- (BOOL)updateObject:(id<SnailDatabaseProtocol>)object setKeys:(NSString *)setKeys byCriteria:(NSString *)criteria {
    __block BOOL result = NO;
    [self inDatabase:object database:^(FMDatabase *db) {
        NSString *sql = [NSString updateTable:object.sl_tableName set:setKeys criteria:criteria];
        result = [db executeUpdate:sql];
        if (!result) {
            printError(@"update failed", object.sl_tableName, db.lastErrorMessage, sql);
        }
    }];
    return result;
}
...
- (BOOL)updateObject:(id<SnailDatabaseProtocol>)object sqlWithFormat:(NSString *)format, ... {
    ...
    [self inDatabase:object database:^(FMDatabase *db) {
        result = [db executeUpdate:sql];
        if (!result) {
            printError(@"(sqlFormat)update failed", object.sl_tableName, db.lastErrorMessage, sql);
        }
    }];
    return result;
}
...
- (NSArray *)selectObject:(id<SnailDatabaseProtocol>)object byCriteriaWithFormat:(NSString *)format, ... {
    NSString *criteria = nil;
    if (format) {
        va_list args;
        va_start(args, format);
    ...
    return [self selectObject:object sql:[NSString selectTable:object.sl_tableName criteria:criteria]];
}

- (NSArray *)selectObject:(id<SnailDatabaseProtocol>)object orderKey:(NSString *)orderKey descSort:(BOOL)descSort limitCount:(NSInteger)limitCount byCriteriaWithFormat:(NSString *)format, ... {
    NSString *criteria = nil;
    if (format) {
        va_list args;
        va_start(args, format);
    ...
    return [self selectObject:object sql:[NSString selectTable:object.sl_tableName criteria:criteria orderKey:orderKey limitCount:limitCount isDesc:descSort]];
}
...
- (NSArray *)selectObject:(id<SnailDatabaseProtocol>)object sql:(NSString *)sql {
    NSMutableArray *dictionaries = [NSMutableArray array];
    [self inDatabase:object database:^(FMDatabase *db) {
        NSMutableArray *columnNames = [NSMutableArray array], *columnTypes = [NSMutableArray array];
        FMResultSet *res = [db getTableSchema:object.sl_tableName];
        while ([res next]) {
...
/// 自动映射属性并创建
- (BOOL)mapColumnCreateTableWithObject:(id<SnailDatabaseProtocol>)object appendPrimaryKeyAndUniqueKeys:(NSString *)description {
...
- (BOOL)mapColumnSaveObject:(id<SnailDatabaseProtocol>)object {
    ...
    NSString *sql = [NSString replaceIntoTable:object.sl_tableName column:columnString columnCount:values.count];
    return [self saveObject:object values:values sql:sql];
}
...
- (BOOL)mapColumnSaveIgnoreObject:(id<SnailDatabaseProtocol>)object {
    ...
    NSString *sql = [NSString ignoreIntoTable:object.sl_tableName column:columnString columnCount:values.count];
    return [self saveObject:object values:values sql:sql];
}
...
- (NSArray *)mapColumnSelectObject:(id<SnailDatabaseProtocol>)object byCriteriaWithFormat:(NSString *)format, ... {
    NSString *criteria = nil;
    if (format) {
        va_list args;
        va_start(args, format);
    ...
    NSString *sql = [NSString selectTable:object.sl_tableName criteria:criteria];
    return [self mapColumnSelectObject:object sql:sql];
}
...
- (NSArray *)mapColumnSelectObject:(id<SnailDatabaseProtocol>)object
    ...
    NSString *sql = [NSString selectTable:object.sl_tableName criteria:criteria orderKey:orderKey limitCount:limitCount isDesc:descSort];
    return [self mapColumnSelectObject:object sql:sql];
}

/// 自动映射属性并查询数据
- (NSArray *)mapColumnSelectObject:(id<SnailDatabaseProtocol>)object sql:(NSString *)sql {
...
- (NSString *)getColumnDescription:(id<SnailDatabaseProtocol>)object {
    NSMutableString *columnString = [NSMutableString string];
    NSArray *properties = [self getProperties:object];
    NSArray *propertyNames = properties.firstObject, *propertyTypes = properties.lastObject;
    [propertyNames enumerateObjectsUsingBlock:^(NSString *name, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (NSArray *)getProperties:(id<SnailDatabaseProtocol>)object {
    // 使用数组有序存储属性及对应的类型
...
- (NSSet *)getIgnoreProperties:(id<SnailDatabaseProtocol>)object {
    ...
    if ([object conformsToProtocol:@protocol(SnailDatabaseProtocol)] &&
        [object respondsToSelector:@selector(sl_mapColumnIgnoreKeys)]) {
        id <SnailDatabaseProtocol>obj = (id<SnailDatabaseProtocol>)object;
        NSSet *ignoreKeys = [NSSet setWithArray:obj.sl_mapColumnIgnoreKeys];
